...
### Project contents
The microservice application has a health endpoint which is accessible at `<host>:<port>/teetimepwcc/health`. The context root is set in the `src/main/webapp/WEB-INF/ibm-web-ext.xml` file. The ports are set in the pom.xml file and exposed to the CLI in the cli-config.yml file.
...
### Local Dependencies
Install the local dependencies.  This project is basically a servlet REST wrapper to core tee time modules.  The tee time dependencies are in other jar files, namely webscraper and teetime.  To install the dependencies, run
this shell script found in the teetimepwcc directory:

`./install_dependencies.sh`

Note that this installs dependencies int the local maven repo normally at ~/.m2... as well as a local maven repo in the project itself (needed for a docker/kube deployment)

Note that the project's pom.xml also needs to list any dependencies like commons-codec, commons, orj.son, etc.
...
Note that you need to run `mvn install` before `cf push` to update dependencies, etc. before pushing the app.
